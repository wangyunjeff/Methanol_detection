...
writer1 = SummaryWriter('runs_SIN_difTest/MLP100-triplet1')
...
def load_triplet_dataset(filepath, focus_labels, focus_factors):
...
# 加载数据集
train_dataset = load_triplet_dataset('./data/SIN_train_data_with_labels_difTest.csv', focus_labels, focus_factors)
test_dataset = load_triplet_dataset('./data/SIN_test_data_with_labels_difTest.csv', focus_labels, focus_factors)
...
num_classes = len(torch.unique(torch.cat([train_dataset.labels, test_dataset.labels])))
# print(len(labels.unique()))
# 创建模型实例，可以调整 dropout_rate 的值
import matplotlib.pyplot

model = SimpleMLP_triplet(input_size, hidden_size, dropout_rate=0.0).to(
    device)  # Make sure it's suitable for triplet output
# 计算类别权重
...
train_triplet_model(model, train_loader, test_loader, triplet_loss, optimizer, num_epochs=10000)
